...
import {
    ...
    Flag, CalendarMonth, Person, FamilyRestroom,
    BugReport, Storage, Visibility, AccountCircle,
...
// Web nav — NO exam taking, NO pw-test, NO live-monitor, NO override panel
const navConfig = {
    technical: [
...
export default function DashboardLayout({ children }) {
    ...
    const drawer = (
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
            {/* Brand */}
            <Box sx={{ p: collapsed ? 1.5 : 2.5, display: 'flex', alignItems: 'center', gap: 1.5, justifyContent: collapsed ? 'center' : 'flex-start', minHeight: 64 }}>
                <Box sx={{ width: 40, height: 40, borderRadius: '12px', flexShrink: 0, background: 'linear-gradient(135deg, #6C63FF, #00D9FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(108,99,255,0.3)' }}>
                    <Security sx={{ fontSize: 22, color: '#fff' }} />
                </Box>
                {!collapsed && (
                    <Box sx={{ overflow: 'hidden', whiteSpace: 'nowrap' }}>
                        <Typography variant="subtitle1" fontWeight={700} sx={{ lineHeight: 1.2 }}>ProctorWatch</Typography>
                        <Typography variant="caption" color="text.secondary">Web Portal</Typography>
                    </Box>
                )}
            </Box>
                    ...
                    <Tooltip key={item.label} title={collapsed ? item.label : ''} placement="right" arrow>
                        <ListItemButton
                            ...
                            sx={{
                                ...
                                '&:hover': { background: muiTheme.palette.action.hover },
                            }}
                        >
                            <ListItemIcon sx={{ minWidth: collapsed ? 0 : 40, justifyContent: 'center', color: location.pathname === item.path ? muiTheme.palette.primary.main : 'text.secondary' }}>
                                {item.icon}
                            </ListItemIcon>
                            {!collapsed && (
                                <ListItemText primary={item.label} primaryTypographyProps={{ fontSize: '0.875rem', fontWeight: location.pathname === item.path ? 600 : 400, noWrap: true }} />
                            )}
                        </ListItemButton>
                    </Tooltip>
                ))}
            ...
            {!collapsed ? (
                <Box sx={{ p: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1.5, borderRadius: 2, background: muiTheme.palette.action.hover }}>
                        <Avatar src={user.profile_photo_url} sx={{ bgcolor: roleColors[user.role], width: 40, height: 40, flexShrink: 0 }}>
                            {(user.full_name || user.username)?.[0]?.toUpperCase()}
                        </Avatar>
                        <Box sx={{ overflow: 'hidden', flex: 1 }}>
                            <Typography variant="body2" fontWeight={600} noWrap>{user.full_name || user.username}</Typography>
                            <Chip size="small" label={roleLabels[user.role]} sx={{ height: 20, fontSize: '0.65rem', bgcolor: `${roleColors[user.role]}22`, color: roleColors[user.role] }} />
                        </Box>
                ...
                <Box sx={{ p: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                    <Tooltip title={user.full_name || user.username} placement="right">
                        <Avatar src={user.profile_photo_url} sx={{ bgcolor: roleColors[user.role], width: 36, height: 36 }}>
                            {(user.full_name || user.username)?.[0]?.toUpperCase()}
                        </Avatar>
                    </Tooltip>
        ...
        <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
            ...
            <Box component="nav" sx={{ width: { md: currentWidth }, flexShrink: { md: 0 }, transition: 'width 200ms ease' }}>
                ...
                <Drawer variant="permanent"
                    sx={{ display: { xs: 'none', md: 'block' }, '& .MuiDrawer-paper': { width: currentWidth, transition: 'width 200ms ease', overflowX: 'hidden' } }}
                    open>
                    {drawer}
                </Drawer>
            </Box>
            ...
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <AppBar position="static" elevation={0} color="transparent" sx={{ borderBottom: `1px solid ${muiTheme.palette.divider}` }}>
                    <Toolbar>
                        <IconButton onClick={() => setMobileOpen(true)} sx={{ mr: 2, display: { md: 'none' }, color: 'text.primary' }}>
                            <MenuIcon />
                        </IconButton>
                        <Tooltip title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
                            <IconButton onClick={() => setCollapsed(!collapsed)} sx={{ mr: 2, display: { xs: 'none', md: 'flex' }, color: 'text.primary' }}>
                                <MenuIcon />
                            </IconButton>
                        </Tooltip>
                        ...
                        <Chip
                            ...
                            size="small"
                            sx={{ bgcolor: `${roleColors[user.role]}15`, color: roleColors[user.role], border: `1px solid ${roleColors[user.role]}30`, fontWeight: 600 }}
                        />
                    </Toolbar>
                </AppBar>

                <Box sx={{ flex: 1, overflow: 'auto', p: 3, background: mode === 'dark' ? 'radial-gradient(ellipse at 50% 0%, rgba(108,99,255,0.04) 0%, transparent 70%)' : 'radial-gradient(ellipse at 50% 0%, rgba(108,99,255,0.03) 0%, transparent 70%)' }}>
                    {children}
                </Box>
            </Box>
        </Box>
